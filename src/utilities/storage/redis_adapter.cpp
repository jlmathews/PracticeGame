...
namespace PGame
{
    ...
    std::string RedisAdapter::GetValue(std::string table, std::string key)
    {
        ...
        if(value)
        {
            spdlog::info("Key {}, Value: {}", key, value.value());
    ...
    {
        spdlog::info("{}: Key {}, Value: {}", __func__, key, value);
    ...
    {
        spdlog::info("{}: Pattern {}", __func__, pattern);
